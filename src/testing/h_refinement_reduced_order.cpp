...
HRefinementStudyReducedOrder<dim, nstate>::HRefinementStudyReducedOrder(
...
template <int dim, int nstate>
Parameters::AllParameters HRefinementStudyReducedOrder<dim,nstate>::reinit_params_and_refine(int refinement) const
{
...
template <int dim, int nstate>
void HRefinementStudyReducedOrder<dim,nstate>::calculate_Lp_error_at_final_time_wrt_function(double &Lp_error_density,
...
template <int dim, int nstate>
int HRefinementStudyReducedOrder<dim, nstate>::run_test() const
{
    ...
    for (int refinement = 0; refinement < n_calculations; ++refinement){
        ...
        pcout << "Finished flowsolver FOM" << std::endl;
        Parameters::AllParameters ROM_param = params;
        ...
        std::unique_ptr<FlowSolver::FlowSolver<dim,nstate>> ROM_flow_solver = FlowSolver::FlowSolverFactory<dim,nstate>::select_flow_case(&Entropy_ROM_param_const, parameter_handler);
        try {
            static_cast<void>(ROM_flow_solver->run());
        } catch (double end) {
            pcout << "ESROM Failed at t = " << end << std::endl;
        }
        std::string path = params.reduced_order_param.path_to_search; //Search specified directory for files containing "solutions_table"
        ...
        const double ROM_final_time_actual = ROM_flow_solver->ode_solver->current_time;
        ...
        double ROM_L1_error_density=0;
        double ROM_L1_error_pressure=0;
        calculate_Lp_error_at_final_time_wrt_function(ROM_L1_error_density, ROM_L1_error_pressure, ROM_flow_solver->dg, params,ROM_final_time_actual, 1);
        double ROM_L2_error_density =0;
        double ROM_L2_error_pressure=0;
        calculate_Lp_error_at_final_time_wrt_function(ROM_L2_error_density,ROM_L2_error_pressure, ROM_flow_solver->dg, params,ROM_final_time_actual, 2);
        double ROM_Linfty_error_density = 0;
        double ROM_Linfty_error_pressure = 0;
        calculate_Lp_error_at_final_time_wrt_function(ROM_Linfty_error_density,ROM_Linfty_error_pressure, ROM_flow_solver->dg, params,ROM_final_time_actual, -1);
        pcout << "Computed density errors are: " << std::endl
              << "    ROM L1:      " << ROM_L1_error_density << std::endl
              << "    ROM L2:      " << ROM_L2_error_density << std::endl
              << "    ROM Linfty:  " << ROM_Linfty_error_density << std::endl;

        const double dt = flow_solver_case->get_constant_time_step(ROM_flow_solver->dg);
        ...
        convergence_table_density.add_value("ROM_L1_error_density",ROM_L1_error_density);
        convergence_table_density.set_precision("ROM_L1_error_density", 16);
        convergence_table_density.evaluate_convergence_rates("ROM_L1_error_density", "n_cells", dealii::ConvergenceTable::reduction_rate_log2, PHILIP_DIM);
        convergence_table_density.add_value("ROM_L2_error_density",ROM_L2_error_density);
        convergence_table_density.set_precision("ROM_L2_error_density", 16);
        convergence_table_density.evaluate_convergence_rates("ROM_L2_error_density", "n_cells", dealii::ConvergenceTable::reduction_rate_log2, PHILIP_DIM);
        convergence_table_density.add_value("ROM_Linfty_error_density",ROM_Linfty_error_density);
        convergence_table_density.set_precision("ROM_Linfty_error_density", 16);
        convergence_table_density.evaluate_convergence_rates("ROM_Linfty_error_density", "n_cells", dealii::ConvergenceTable::reduction_rate_log2, PHILIP_DIM);
        if (params.ode_solver_param.ode_solver_type == Parameters::ODESolverParam::ODESolverEnum::rrk_explicit_solver) {
            const double gamma_agg = final_time_actual / (dt * ROM_flow_solver->ode_solver->current_iteration);
        ...
        convergence_table_pressure.add_value("ROM_L1_error_pressure",ROM_L1_error_pressure);
        convergence_table_pressure.set_precision("ROM_L1_error_pressure", 16);
        convergence_table_pressure.evaluate_convergence_rates("ROM_L1_error_pressure", "n_cells", dealii::ConvergenceTable::reduction_rate_log2, PHILIP_DIM);
        convergence_table_pressure.add_value("ROM_L2_error_pressure",ROM_L2_error_pressure);
        convergence_table_pressure.set_precision("ROM_L2_error_pressure", 16);
        convergence_table_pressure.evaluate_convergence_rates("ROM_L2_error_pressure", "n_cells", dealii::ConvergenceTable::reduction_rate_log2, PHILIP_DIM);
        convergence_table_pressure.add_value("ROM_Linfty_error_pressure",ROM_Linfty_error_pressure);
        convergence_table_pressure.set_precision("ROM_Linfty_error_pressure", 16);
        convergence_table_pressure.evaluate_convergence_rates("ROM_Linfty_error_pressure", "n_cells", dealii::ConvergenceTable::reduction_rate_log2, PHILIP_DIM);
...
#if PHILIP_DIM!=1
    template class HRefinementStudyReducedOrder<PHILIP_DIM,PHILIP_DIM+2>;
#endif
}
}