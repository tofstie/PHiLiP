...
void DGHyper<dim,nstate,real,MeshType>::assemble_hyper_reduced_residual (
    ...
    //     if (QtxF_file.is_open()){
    //         QtxF_file << QtxF_eig.format(CSVFormat);
    //     }
    //     QtxF_file.close();
    // }
    // {
    //     std::ofstream BExFB_file("BExFB"+std::to_string(this->current_time)+".txt");
    //     Eigen::MatrixXd BExFB_eig = epetra_to_eig_matrix(BExFB);
    //     if (BExFB_file.is_open()){
    //         BExFB_file << BExFB_eig.format(CSVFormat);
    //     }
    //     BExFB_file.close();
    // }
    //
    ...
    // Add flux at boundary interpolated
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_subface_term_and_build_operators(
    typename dealii::DoFHandler<dim>::active_cell_iterator cell,
    typename dealii::DoFHandler<dim>::active_cell_iterator neighbor_cell,
    const dealii::types::global_dof_index                  current_cell_index,
    const dealii::types::global_dof_index                  neighbor_cell_index,
 ...
 **************************************************/
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_auxiliary_residual()
{
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_volume_term_auxiliary_equation(
    const std::vector<dealii::types::global_dof_index> &current_dofs_indices,
    const unsigned int poly_degree,
    OPERATOR::basis_functions<dim,2*dim,real> &soln_basis,
    OPERATOR::basis_functions<dim,2*dim,real> &flux_basis,
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_boundary_term_auxiliary_equation(
    const unsigned int iface,
    const dealii::types::global_dof_index current_cell_index,
    const unsigned int poly_degree,
    const unsigned int boundary_id,
...
/*********************************************************************************/
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_face_term_auxiliary_equation(
    ...
    for(unsigned int istate_counter = 0; istate_counter < (unsigned int) nstate; ++istate_counter ) {
        ...
        for (unsigned int row = 0; row < n_quad_pts; ++row) {
            for (unsigned int col = 0; col < n_quad_pts_1D; ++col) {
                ...
                int indices = cell_dofs_indices[col+n_quad_pts_1D*id_counter+n_quad_pts*istate_counter];

                global_Q.InsertGlobalValues(cell_dofs_indices[row+n_quad_pts*istate_counter],1.,&val,&indices);
            }
            if(row == n_quad_pts_1D-1) id_counter++;
        }
    }
    std::ofstream hyper_Q_file("hyper_Q.txt");
    std::ofstream global_Q_file("Global_Q.txt");
    global_Q.FillComplete(global_map,global_map);
    global_Q.Print(global_Q_file);
    Epetra_CrsMatrix hyper_reduced_Q = this->calculate_hyper_reduced_Q(global_Q);
    hyper_reduced_Q.Print(hyper_Q_file);
    */
    //For each state we:
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_boundary_term_derivatives(
    typename dealii::DoFHandler<dim>::active_cell_iterator /*cell*/,
    const dealii::types::global_dof_index /*current_cell_index*/,
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_volume_term_derivatives(
    typename dealii::DoFHandler<dim>::active_cell_iterator /*cell*/,
    const dealii::types::global_dof_index /*current_cell_index*/,
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_face_term_derivatives(
    typename dealii::DoFHandler<dim>::active_cell_iterator    /*cell*/,
    const dealii::types::global_dof_index                     /*current_cell_index*/,
    const dealii::types::global_dof_index                     /*neighbor_cell_index*/,
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::assemble_volume_term_explicit(
    typename dealii::DoFHandler<dim>::active_cell_iterator /*cell*/,
    const dealii::types::global_dof_index /*current_cell_index*/,
    const dealii::FEValues<dim,dim> &/*fe_values_vol*/,
    const std::vector<dealii::types::global_dof_index> &/*cell_dofs_indices*/,
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::allocate_dual_vector()
{
    //Do nothing.
}
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::calculate_global_entropy(bool use_quad_entropy)
{
    ...
    for (auto current_cell = this->dof_handler.begin_active(); current_cell != this->dof_handler.end(); ++current_cell, ++metric_cell) {
        ...
        if(use_quad_entropy) {
            // Here I am now using global QUADRATURE entropy as I am making my projection operator the full one.
            for(int istate=0; istate<nstate; istate++) {
                for(unsigned int iquad = 0; iquad < n_quad_pts; iquad++){
                    this->global_entropy[current_dofs_indices[istate*n_shape_fns + iquad]] = entropy_var_at_q[istate][iquad];
                }
            }
        } else {
            for(int istate=0; istate<nstate; istate++){
    ...
    for(unsigned int i = 0 ; i < this->global_entropy.size(); i++){
        if (this->global_entropy.in_local_range(i)){
            file << std::setprecision(16) << this->global_entropy[i]  << '\n';
        }
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim, nstate, real, MeshType>::location2D(
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::calculate_projection_matrix(dealii::TrilinosWrappers::SparseMatrix &V)
{
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::calculate_projection_matrix(Epetra_CrsMatrix &LHS, Epetra_CrsMatrix &LeV)
{
Epetra_MpiComm comm( MPI_COMM_WORLD );
    const unsigned int init_grid_degree = this->high_order_grid->fe_system.tensor_degree();
    const auto mapping = (*(this->high_order_grid->mapping_fe_field));
    ...
    Epetra_Map row_map = LeV.RowMap();
    Epetra_Map domain_map = LeV.DomainMap();
    ...
    for (auto current_cell = this->dof_handler.begin_active(); current_cell != this->dof_handler.end(); ++current_cell, ++metric_cell) {
        ...
        const int i_fele = current_cell->active_fe_index();
        const unsigned int poly_degree = i_fele;
        const unsigned int curr_grid_degree   = this->high_order_grid->fe_system.tensor_degree();
        const dealii::FESystem<dim,dim> &current_fe_ref = this->fe_collection[i_fele];
        ...
        soln_basis_int.build_1D_volume_operator(oneD_fe_ref,this->oneD_quadrature_collection[poly_degree]);
        dealii::FullMatrix<double> chi_v_tensor(n_quad_pts);
       // std::cout << "Local Integration" << std::endl;
        chi_v_tensor = soln_basis_int.tensor_product_nstate(1,soln_basis_int.oneD_vol_operator,soln_basis_int.oneD_vol_operator,soln_basis_int.oneD_vol_operator);
        //Wchi_v_tensor.print_formatted(std::cout,3,true,0,0);
        OPERATOR::metric_operators<real, dim, 2*dim> metric_oper(nstate, i_fele, curr_grid_degree);
        metric_oper.build_determinant_volume_metric_Jacobian(
                        n_quad_pts, n_grid_nodes,
                        mapping_support_points,
                        mapping_basis);

        for(unsigned int cell_row_idx = 0; cell_row_idx < n_quad_pts; ++cell_row_idx) {
            ...
            for(int istate = 0; istate < nstate; ++istate) {
                ...
                for(unsigned int cell_col_idx = 0; cell_col_idx < n_quad_pts; ++cell_col_idx) {
                   // if(this->reduced_mesh_weights[current_cell_index*n_quad_pts + cell_col_idx] == 0) continue;
                    const int global_col_idx = (int)current_dofs_indices[cell_col_idx+istate*n_quad_pts];
                    const double value = chi_v_tensor[cell_row_idx][cell_col_idx];
                    //std::cout << "Row :" << std::to_string(global_row_idx) << " Col :" << std::to_string(global_col_idx) <<  " Val :" << value << std::endl;
                    chi_v.InsertGlobalValues(global_row_idx,1,&value,&global_col_idx);
                }
            }
        }
    }
    ...
    int mult2;
    mult2 = EpetraExt::MatrixMatrix::Multiply(LHSVt,false,chi_v,true,LHSVtChivT);
    ...
    int mult3;
    std::ofstream Projec_file("mult3.txt");
...
template <int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::calculate_ROM_projected_entropy(dealii::TrilinosWrappers::SparseMatrix &/*V*/)
{
    ...
    dealii::TrilinosWrappers::SparseMatrix V_dealii;
    V_dealii.reinit(*this->galerkin_basis);
    std::ofstream galerkin_file("Basis_In_DG_Object.txt");
    this->galerkin_basis->Print(galerkin_file);
    this->projected_entropy= this->solution;
    this->projection_matrix.vmult(temp_val,this->global_entropy);
    V_dealii.vmult(this->projected_entropy,temp_val);
    // Global Face Entropy
...
Epetra_CrsMatrix DGHyper<dim, nstate, real, MeshType>::calculate_hyper_reduced_Q(Epetra_CrsMatrix &Global_Q, Epetra_CrsMatrix &hyper_Vt, const int idim) {
    ...
    EpetraExt::MatrixMatrix::Multiply(galerkin_basis_matrix,true,Global_Q,false,first_temp);

    Epetra_CrsMatrix second_temp(Epetra_DataAccess::Copy,first_temp_row_map,galerkin_basis_matrix.NumGlobalCols());
    EpetraExt::MatrixMatrix::Multiply(first_temp,false,galerkin_basis_matrix,false,second_temp);

    Epetra_CrsMatrix third_temp(Epetra_DataAccess::Copy,third_temp_row_map,second_temp.NumGlobalCols());
    EpetraExt::MatrixMatrix::Multiply(proj_epetra,true,second_temp,false,third_temp);
    ...
    for (auto current_cell = this->dof_handler.begin_active(); current_cell != this->dof_handler.end(); ++current_cell) {
        ...
        // Current reference element related to this physical cell
        const dealii::FESystem<dim,dim> &current_fe_ref = this->fe_collection[fe_index_curr_cell];
        ...
        for (int iquad = 0; iquad < n_quad_pts; ++iquad) {
            ...
            int NumEntries =0;
            hyper_reduced_Q.ExtractGlobalRowCopy(iquad+current_cell_index*n_quad_pts,hyper_reduced_Q.NumGlobalCols(),NumEntries,global_row.data(),indicies.data());
            for (int istate = 0; istate < nstate; ++istate) {
                const int GlobalRowIndex = current_dofs_indices[iquad+istate*n_quad_pts];
    ...
    */
    ...
    int NumEntries =0;
    const static Eigen::IOFormat CSVFormat(Eigen::FullPrecision, Eigen::DontAlignCols, ", ", "\n");
    std::ofstream qt_before_re_file("qt_quad_"+std::to_string(idim)+".txt");
    Eigen::MatrixXd qt_before_re = epetra_to_eig_matrix(hyper_reduced_Q);
    if (qt_before_re_file.is_open()){
        qt_before_re_file << qt_before_re.format(CSVFormat);
    }
    qt_before_re_file.close();
    ...
    for(int i_quad = 0; i_quad < hyper_reduced_Q.NumGlobalRows();i_quad++)
    {
        ...
        for(int entry = 0; entry < NumEntries;entry++)
        {
            ...
            //const double neg_val = dof_row == dof_col ? val : -1.0*val;
...
void DGHyper<dim, nstate, real, MeshType>::calculate_convective_flux_matrix(
    ...
    for (auto current_cell = this->dof_handler.begin_active(); current_cell != this->dof_handler.end(); ++current_cell, ++metric_cell) {
        ...
        for (unsigned int iquad=0; iquad<n_quad_pts; ++iquad) {
            ...
            if (this->all_parameters->use_split_form || this->all_parameters->use_curvilinear_split_form){
                ...
                std::array<real,nstate> entropy_var;
                for(int istate=0; istate<nstate; istate++){
                    entropy_var[istate] = this->projected_entropy[current_dofs_indices[istate*n_quad_pts+iquad]];
                    //entropy_var[istate] = this->global_entropy[current_dofs_indices[istate*n_quad_pts+iquad]];
                }
                std::array<real,nstate> soln_state;
                soln_state = this->pde_physics_double->compute_conservative_variables_from_entropy_variables (entropy_var);
                auto flux_metric_cell = this->high_order_grid->dof_handler_grid.begin_active();
                std::array<std::vector<real>,dim> flux_mapping_support_points;
                auto flux_cells_start = std::chrono::high_resolution_clock::now();
                for (auto flux_current_cell = this->dof_handler.begin_active(); flux_current_cell != this->dof_handler.end(); ++flux_current_cell, ++flux_metric_cell) {
                    ...
                    flux_metric_cell->get_dof_indices (flux_metric_dofs_indices);
    ...
    auto this_cells_duration = std::chrono::duration_cast<std::chrono::microseconds>(this_cells_end - this_cells_start);
    ...
    int global_boundary_quad_point_index = 1;
                    ...
                    for(unsigned int flux_quad=0; flux_quad<n_quad_pts; ++flux_quad) {
                        for (int ref_dim =0;ref_dim<dim;ref_dim++) {
                            ...
                            for(int istate=0; istate<nstate; istate++){
                                ...
                                //write into reference Hadamard flux matrix
                                if(isnan(conv_ref_flux_2pt[ref_dim])) conv_ref_flux_2pt[ref_dim] = 0;
                                int flux_column = (global_boundary_quad_point_index+1)*nstate + global_size-(nstate-istate-1)-1;
                                int quad_point = (iface == 0) ? 0 : n_quad_pts-1;
                                this->boundary_term_map[flux_column-global_size] = current_dofs_indices[quad_point + (nstate-istate-1)*n_quad_pts];
                                if(ref_dim == 0) {
                                    ...
                                    FBx.InsertGlobalValues(flux_column-global_size,1,&conv_ref_flux_2pt[ref_dim],&current_flux_dofs_index);
                                } else if(ref_dim == 1) {
...
template<int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim, nstate, real, MeshType>::construct_global_Q(Epetra_CrsMatrix &Qx,Epetra_CrsMatrix &Qy,Epetra_CrsMatrix &Qz,bool skew_symmetric) {
    const int global_size = this->solution.size()/nstate;
    Epetra_MpiComm comm(MPI_COMM_WORLD);
    Epetra_Map global_map(global_size,0,comm);
   // const unsigned int n_dofs_cell_estimate = this->fe_collection[this->all_parameters->flow_solver_param.poly_degree].dofs_per_cell;
    ...
    OPERATOR::basis_functions<dim,2*dim,real> flux_basis_int(1, this->max_degree, init_grid_degree);
    OPERATOR::basis_functions<dim,2*dim,real> flux_basis_ext(1, this->max_degree, init_grid_degree);
    OPERATOR::face_integral_basis<dim,2*dim,real> face_intergral(1,this->max_degree, init_grid_degree);
    OPERATOR::local_basis_stiffness<dim,2*dim,real> flux_basis_stiffness(1, this->max_degree, init_grid_degree, true);
    OPERATOR::vol_projection_operator<dim,2*dim,real> soln_basis_projection_oper_int(1, this->max_degree, init_grid_degree);
    OPERATOR::vol_projection_operator<dim,2*dim,real> soln_basis_projection_oper_ext(1, this->max_degree, init_grid_degree);
    OPERATOR::mapping_shape_functions<dim,2*dim,real> mapping_basis(1, init_grid_degree, init_grid_degree);
    ...
    for (auto current_cell = this->dof_handler.begin_active(); current_cell != this->dof_handler.end(); ++current_cell, ++metric_cell) {
        ...
        if(poly_degree != soln_basis_int.current_degree){
            ...
            flux_basis_ext.current_degree = poly_degree;
            mapping_basis.current_degree  = poly_degree;
            face_intergral.current_degree = poly_degree;
            face_intergral.build_1D_surface_operator(this->oneD_fe_collection_flux[this->max_degree], this->oneD_face_quadrature);
            const unsigned int grid_degree = this->high_order_grid->fe_system.tensor_degree();
            this->reinit_operators_for_cell_residual_loop(poly_degree, poly_degree, grid_degree,
                                                            soln_basis_int, soln_basis_ext,
        ...
        for (unsigned int i =0; i < oneD_vol_quad_weights.size(); i++) {
            W.set(i,i,oneD_vol_quad_weights[i]);
        }
        dealii::FullMatrix<double> local_S(n_quad_pts);

        //flux_basis_stiffness_skew_symm_oper_sparse[0].print_formatted(std::cout, 14, true, 10, "0", 1., 0.);
         const int global_size = this->solution.size();
         Epetra_MpiComm comm(MPI_COMM_WORLD);
         Epetra_Map global_map(global_size,0,comm);
        for(int idim = 0; idim < dim;idim++) {
         ...
         //     for (unsigned int row = 0; row < n_quad_pts; ++row) {
         //         for (unsigned int col = 0; col < n_quad_pts_1D; ++col) {
         //             double val = flux_basis_stiffness_skew_symm_oper_sparse[idim](row,col);
         //             int indices = current_cell_index*n_quad_pts+col+n_quad_pts_1D*id_counter;
        ...
        if (dim == 1) {
            ...
            for (unsigned int idim=0; idim < dim; ++idim) {
                int starting_quad = 0;
                int ending_quad = n_quad_pts_1D-1;
                int bonus = 0;
        ...
        if(dim != 1) {
            ETBx *= 1./(n_quad_pts_face);
            ETBy *= 1./(n_quad_pts_face);
        }
        // ETBx.print_formatted(std::cout, 14, true, 10, "0", 1., 0.);
        // BEx.print_formatted(std::cout, 14, true, 10, "0", 1., 0.);
        for (unsigned int iface=0; iface < dealii::GeometryInfo<dim>::faces_per_cell; ++iface) {
            auto current_face = current_cell->face(iface);
            if ((current_face->at_boundary() && !current_cell->has_periodic_neighbor(iface))) {
                for(unsigned int i_quad_oneD = 0; i_quad_oneD < n_quad_pts_face; i_quad_oneD++) {
                    ...
                    const double neg_one_double = -1.0;
                ...
                for(unsigned int i_quad_oneD = 0; i_quad_oneD < n_quad_pts_face; i_quad_oneD++) {
                    if(i_dim == 0) {
                        ...
                        if(local_col <= (n_quad_pts_1D-1)) {
                            global_point = n_quad_pts_1D*local_col;
                        } else if(local_col > n_quad_pts_1D-1) {
                            global_point = n_quad_pts_1D*(local_col-n_quad_pts_1D)+(n_quad_pts_1D-1);
                        } else {
                        ...
                        unsigned int local_col = n_quad_pts_face*i_face_1D+i_quad_oneD;
                        unsigned int global_point;
                        if(local_col < n_quad_pts_1D) {
                            global_point = local_col;
                        } else {
                            global_point = local_col + (n_quad_pts_1D)*(n_quad_pts_1D-2);
                        }
    ...
    if (skew_symmetric) {
        ...
        Qx.Print(QxFile);
        std::ofstream QyFile("(Q-QT)y_file.txt");
        Qy.Print(QyFile);
    } else {
        std::ofstream QxFile("Qx_file.txt");
        Qx.Print(QxFile);
        std::ofstream QyFile("Qy_file.txt");
        Qy.Print(QyFile);
    }
...
real DGHyper<dim, nstate, real, MeshType>::evaluate_penalty_scaling_hyper (
...
bool DGHyper<dim, nstate, real, MeshType>::current_cell_should_do_the_work_hyper (
...
template<int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim, nstate, real, MeshType>::assemble_volume_basis() {
    //std::cout <<"Chi" << std::endl;
    Epetra_Map domain_map = this->global_mass_matrix.trilinos_matrix().DomainMap();
    Epetra_Map row_map = this->global_mass_matrix.trilinos_matrix().RowMap();
    Epetra_Map col_map = this->global_mass_matrix.trilinos_matrix().ColMap();
    const unsigned int init_grid_degree = this->high_order_grid->fe_system.tensor_degree();
    const auto mapping = (*(this->high_order_grid->mapping_fe_field));
    dealii::hp::MappingCollection<dim> mapping_collection(mapping);
    ...
    Epetra_CrsMatrix chi_v_global(Epetra_DataAccess::Copy,row_map,col_map,this->global_mass_matrix.trilinos_matrix().GlobalMaxNumEntries());

    //std::cout <<"Chi cell loop" << std::endl;
    for (auto current_cell = this->dof_handler.begin_active(); current_cell != this->dof_handler.end(); ++current_cell) {
        if (!current_cell->is_locally_owned()) continue;
        //const dealii::types::global_dof_index current_cell_index = current_cell->active_cell_index();
        std::vector<dealii::types::global_dof_index> neighbor_dofs_indices;
        ...
        const unsigned int n_dofs_curr_cell = current_fe_ref.n_dofs_per_cell();
        const unsigned int n_quad_pts = this->volume_quadrature_collection[poly_degree].size();
        std::vector<dealii::types::global_dof_index> current_dofs_indices(n_dofs_curr_cell);
        current_dofs_indices.resize(n_dofs_curr_cell);
        current_cell->get_dof_indices (current_dofs_indices);
        soln_basis_int.build_1D_volume_operator(oneD_fe_ref,this->oneD_quadrature_collection[poly_degree]);
        dealii::FullMatrix<double> chi_v(n_dofs_curr_cell);
        chi_v = soln_basis_int.tensor_product_nstate(nstate,soln_basis_int.oneD_vol_operator,soln_basis_int.oneD_vol_operator,soln_basis_int.oneD_vol_operator);
        for(unsigned int i_quad_row = 0; i_quad_row < n_quad_pts; ++i_quad_row) {
            //if(this->reduced_mesh_weights[i_quad_row+current_cell_index*n_quad_pts]) continue;
            for(unsigned int i_quad_col = 0; i_quad_col < n_quad_pts; ++i_quad_col) {
                //if(this->reduced_mesh_weights[i_quad_col+current_cell_index*n_quad_pts]) continue;
                for(int istate =0; istate< nstate; istate++) {
                    const int global_row_idx = (int)current_dofs_indices[i_quad_row+istate*n_quad_pts];
                    const int global_col_idx = (int)current_dofs_indices[i_quad_col+istate*n_quad_pts];
                    const double val = chi_v[i_quad_row+istate*n_quad_pts][i_quad_col+istate*n_quad_pts];
                    chi_v_global.InsertGlobalValues(global_row_idx,1,&val,&global_col_idx);
                }
            }
        }
    }
    std::cout <<"Chi made" << std::endl;
    chi_v_global.FillComplete(domain_map,row_map);
    std::ofstream file("Chi_V");
...
Epetra_CrsMatrix  DGHyper<dim,nstate,real,MeshType>::calculate_hyper_reduced_Bx(Epetra_CrsMatrix &Vt, const int idim) {
    ...
    if(this->number_global_boundaries == 2) {
        ...
        const int zero = 0;
        const int one_int = 1;
        B.InsertGlobalValues(0,1,&neg_one,&zero);
        B.InsertGlobalValues(1,1,&one,&one_int);
        B.FillComplete(boundary_map,boundary_map);
...
template<int dim, int nstate, typename real, typename MeshType>
void DGHyper<dim,nstate,real,MeshType>::calculate_boundary_flux() {
    const auto mapping = (*(this->high_order_grid->mapping_fe_field));
    dealii::hp::MappingCollection<dim> mapping_collection(mapping);
    ...
    for (auto current_cell = this->dof_handler.begin_active(); current_cell != this->dof_handler.end(); ++current_cell, ++metric_cell) {
        ...
        for (unsigned int iface=0; iface < dealii::GeometryInfo<dim>::faces_per_cell; ++iface) {
            ...
            auto current_face = current_cell->face(iface);
            if((current_face->at_boundary() && !current_cell->has_periodic_neighbor(iface))){
                unsigned int boundary_id = current_face->boundary_id();;
                ...
                for(int istate = 0; istate < nstate; istate++) {
                    std::vector<real> rhs(n_quad_pts);
                    soln_basis_int.inner_product_surface_1D(iface, conv_flux_dot_normal[istate],
                                                face_quad_weights, rhs,
                                                soln_basis_int.oneD_surf_operator,
                                                soln_basis_int.oneD_vol_operator,
                                                true, -1.0);
                    ...
                    //const int indice = global_boundary_quad_point_index*nstate+istate;
                    for(unsigned int ishape = 0; ishape < n_quad_pts;++ishape ) {
                        current_cell_rhs(istate*n_quad_pts + ishape) += rhs[ishape];
                    }
                }
                std::vector<double> currrernt(n_dofs_curr_cell);
                for (unsigned int i=0; i<n_dofs_curr_cell; ++i) {
                    this->right_hand_side[current_dofs_indices[i]] += current_cell_rhs[i];
                    currrernt[i] = current_cell_rhs[i];
                }
            }
            else {
                // cell has neighbour
                ...
                const unsigned int neighbour_iface = (current_cell->has_periodic_neighbor(iface)) ? current_cell->periodic_neighbor_of_periodic_neighbor(iface) : current_cell->neighbor_of_neighbor(iface);;
                std::vector<dealii::types::global_dof_index> neighbour_dof_indicies(n_dofs_curr_cell);
                neighbour_dof_indicies.resize(n_dofs_curr_cell);
                neighbor_cell->get_dof_indices (neighbour_dof_indicies);
                dealii::Vector<real> neighbor_cell_rhs (n_dofs_curr_cell);
                metric_oper.build_volume_metric_operators(
                    this->volume_quadrature_collection[poly_degree].size(), n_grid_nodes,
                    mapping_support_points,
                    mapping_basis,
                ...
                std::array<std::vector<real>,nstate> projected_entropy_var_surf_ext;
                for(int istate=0; istate<nstate; istate++) {
                    std::vector<real> entropy_var_coeff_int(n_quad_pts);
                    std::vector<real> entropy_var_coeff_ext(n_quad_pts);
                    projected_entropy_var_surf_int[istate].resize(n_face_quad_pts);
                    projected_entropy_var_surf_ext[istate].resize(n_face_quad_pts);

                    for(unsigned int i_shape_fns = 0; i_shape_fns<n_quad_pts; i_shape_fns++){
                        entropy_var_coeff_int[i_shape_fns] = this->projected_entropy[current_dofs_indices[istate*n_quad_pts+i_shape_fns]];
                        entropy_var_coeff_ext[i_shape_fns] = this->projected_entropy[neighbour_dof_indicies[istate*n_quad_pts+i_shape_fns]];
                    }
                    soln_basis_int.matrix_vector_mult_surface_1D(iface,
                                             entropy_var_coeff_int,
                                             projected_entropy_var_surf_int[istate],
                                             soln_basis_int.oneD_surf_operator,
                                             soln_basis_int.oneD_vol_operator);
                    soln_basis_ext.matrix_vector_mult_surface_1D(neighbour_iface,
                                             entropy_var_coeff_ext,
                                             projected_entropy_var_surf_ext[istate],
                                             soln_basis_ext.oneD_surf_operator,
                                             soln_basis_ext.oneD_vol_operator);
                ...
                for (unsigned int iquad=0; iquad<n_face_quad_pts; ++iquad) {
                    ...
                    for(int istate=0; istate<nstate; istate++){
                        // interior RHS
                        std::vector<real> rhs_int(n_quad_pts);
                        std::vector<real> rhs_ext(n_quad_pts);
                        soln_basis_int.inner_product_surface_1D(iface, conv_num_flux_dot_n[istate],
                                                surf_quad_weights, rhs_int,
                                                soln_basis_int.oneD_surf_operator,
                                                soln_basis_int.oneD_vol_operator,
                                                true, -1.0);//adding=true, scaled by factor=-1.0 bc subtract it
                        for(unsigned int ishape = 0; ishape < n_quad_pts; ishape++) {
                            current_cell_rhs(istate*n_quad_pts + ishape) += rhs_int[ishape];
                        }
                        soln_basis_ext.inner_product_surface_1D(neighbour_iface, conv_num_flux_dot_n[istate],
                                                surf_quad_weights, rhs_ext,
                                                soln_basis_ext.oneD_surf_operator,
                                                soln_basis_ext.oneD_vol_operator,
                                                true, 1.0);//adding=true, scaled by factor=-1.0 bc subtract it
                        for(unsigned int ishape = 0; ishape < n_quad_pts; ishape++) {
                            neighbor_cell_rhs(istate*n_quad_pts + ishape) += rhs_ext[ishape];
                        }
                    }
                }
                std::vector<double> neighborrr(n_dofs_curr_cell);
                std::vector<double> currrrent(n_dofs_curr_cell);
                for (unsigned int i=0; i<n_dofs_curr_cell; ++i) {
                    if(std::isnan(neighbor_cell_rhs[i]) || std::isnan(current_cell_rhs[i])) continue;
                    this->right_hand_side[neighbour_dof_indicies[i]] += neighbor_cell_rhs[i];
                    neighborrr[i] = neighbor_cell_rhs[i];
                    this->right_hand_side[current_dofs_indices[i]] += current_cell_rhs[i];
                    currrrent[i] = current_cell_rhs[i];
                }
            }
...
template class DGHyper <PHILIP_DIM, 1, double, dealii::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 2, double, dealii::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 3, double, dealii::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 4, double, dealii::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 5, double, dealii::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 6, double, dealii::Triangulation<PHILIP_DIM>>;

template class DGHyper <PHILIP_DIM, 1, double, dealii::parallel::shared::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 2, double, dealii::parallel::shared::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 3, double, dealii::parallel::shared::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 4, double, dealii::parallel::shared::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 5, double, dealii::parallel::shared::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 6, double, dealii::parallel::shared::Triangulation<PHILIP_DIM>>;

#if PHILIP_DIM!=1
template class DGHyper <PHILIP_DIM, 1, double, dealii::parallel::distributed::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 2, double, dealii::parallel::distributed::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 3, double, dealii::parallel::distributed::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 4, double, dealii::parallel::distributed::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 5, double, dealii::parallel::distributed::Triangulation<PHILIP_DIM>>;
template class DGHyper <PHILIP_DIM, 6, double, dealii::parallel::distributed::Triangulation<PHILIP_DIM>>;
#endif
}