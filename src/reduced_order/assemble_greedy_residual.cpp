...
void AssembleGreedyRes<dim,nstate>::build_problem(){
    ...
    std::vector<int> z_vector(z.begin(), z.end());
    ...
    ///3. Preform Least Squares
        ...
        Eigen::MatrixXd V_target_z_Eigen = V_target(Eigen::placeholders::all,z_vector); 
    ...
    ///6. Fiddle with Sets for all alpha(z_0) = 0 
            ...
            Eigen::MatrixXd V_target_z_Eigen_nnls = V_target(Eigen::placeholders::all,z_vector); 
...
template<int dim, int nstate>
void AssembleGreedyRes<dim,nstate>::build_initial_weights(){
