       ...
       std::shared_ptr<RKTableauBase<dim,real,MeshType>> rk_tableau)
{
    ...
    if ((ode_solver_type == ODEEnum::runge_kutta_solver && dg_input->all_parameters->flow_solver_param.do_calculate_numerical_entropy && rrk_method != RRKEnum::RootFinding)
        || rrk_method == RRKEnum::NumEntropy) {
        ...
        dg_input->evaluate_mass_matrices(false);
